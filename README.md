...
顔検出・美肌加工を初めて使うときは、MediaPipeのモデルファイル（数MB）をCDNから取得します。ボタンを押すまでは通信しませんが、押した瞬間はネット接続が必要です。
...
- 自動配置・美肌加工は顔検出精度に依存し、100%正確ではありません。ズレる/不自然な場合は手動で調整するかスライダーを0に戻してください。
